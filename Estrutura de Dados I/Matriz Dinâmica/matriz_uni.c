  ...
  while(troca == -1){
    ...
    for(int i = 0; i < n; i++){
      if(setor[i * 2 + 0] > setor[(i + 1) * 2 + 0]){
        int aux = setor[(i + 1) * 2 + 0];

        setor[(i + 1) * 2 + 0] = setor[i * 2 + 0];

        setor[i * 2 + 0] = aux;


        int aux2 = setor[(i + 1) * 2 + 1];

        setor[(i + 1) * 2 + 1] = setor[i * 2 + 1];
...
int main(){
  ...
  setor = (float*) malloc (p * 2 * sizeof(float));
    ...
    printf("\nEntre com o salario do Setor %5.1f: ", setor[i * p + 0]);
    scanf("%f", &setor[i * 2 + 1]);
	}
